...
node_modules
  .pnpm
    card@1.0.0(react@16.8.0(typescript@7.0.0)) // 实际上是 .pnpm/card@1.0.0_react@16.8.0_typescript@7.0.0
      node_modules
        card
...
在上面的例子中，`card` peerDep `react`, `react` peerDep `typescript`，由于 `react` 依赖了不同的 `typescript`：

- `project1/package.json` 中的 card 会软链接到 `.pnpm/card@1.0.0(react@16.8.0(typescript@7.0.0))` 这个硬链接
- `project2/package.json` 中的 card 会软链接到 `.pnpm/card@1.0.0(react@16.8.0(typescript@8.0.0))` 这个硬链接

虽然都是 `card@1.0.0`, 但是由于是不同的硬链接，被视为不同的文件，就会打包多次。关于 `peerDependencies` 在 pnpm 中是怎么解析的，可以阅读：[How peers are resolved](https://pnpm.io/how-peers-are-resolved)。
